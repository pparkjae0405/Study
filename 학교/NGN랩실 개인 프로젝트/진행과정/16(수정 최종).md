...
ClientThread
msg = 키보드값
string[] i =  i1, i2, i3, i4를 종합하여 저장할 배열
i1, i2, i3, i4 = 키보드값이 insert일 때 삽입할 값
string[] d = d1, d2를 종합하여 저장할 배열
d1, d2 = 키보드값이 delete일 때 삭제할 조건
string[] u = u1, u2, u3를 종합하여 저장할 배열
u1, u2 = 키보드값이 update일 때 수정할 조건
u3 = 키보드값이 update일 때 수정할 내용
q = for()에 들어갈 조건값
w = 반복할 횟수 받아오는 정수형

ServerThread
Array(배열) + ~~~~~
ai = 키보드 값이 insert일 때 i1~i4로 저장한 배열 값 읽어오기
ad = 키보드 값이 delete일 때 d1~d2로 저장한 배열 값 읽어오기
au = 키보드 값이 update일 때 u1~u3로 저장한 배열 값 읽어오기
